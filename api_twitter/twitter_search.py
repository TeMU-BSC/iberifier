...
class TweetSearchUtil:
    def __init__(
        ...
        self.twitter_cred = searchtweets.load_credentials(
            path_cred, yaml_key=yaml_key, env_overwrite=False
        )
    ...
    def _make_tweets_request(self, rule, max_results=500):
        ...
        tweets = searchtweets.collect_results(
            rule, result_stream_args=self.twitter_cred, max_tweets=max_results
        )
    ...
    def retreive_tweets_by_id(
        self, ids, tweet_fields=None, user_fields=None, expansions=None
    ):
        """
